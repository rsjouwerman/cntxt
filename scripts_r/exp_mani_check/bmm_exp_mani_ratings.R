# load packages
# note, brms only works with R version 3.6
library(brms)       # for running bayesian mixed models
library(dplyr)      # for easy data manipulation
library(emmeans)    # for extracting estimated means
library(ggplot2)    # for plotting
library(here)       # for easy path allocation
library(sjPlot)     # for comparing brm models
library(colorspace)

# load data
out_measure <- 'ratings'
# move data to project directory?
load("~/BouCon_All/RdataManiCheck/raw_rat_single_trial.Rdata")
source(here::here('scripts_r/exp_mani_check/contrasts_exp_mani_check.R'))
dat <- drat
dat$trial <- factor(paste(dat$expPhase, dat$bin1, sep ='_'), 
                    levels = c('hab_1','acq_1','acq_2','acq_3','ext_1','ext_2','ext_3','rei_1','tst_1','tst_2'))

dat$stimType <- factor(dat$stimType)
ggplot(dat, aes(x = trial, y = valueConf)) +
  geom_point(aes(color = stimType)) +
  geom_line(aes(group = idNum, color = stimType))

# means
dat_summary <- dat %>% group_by(trial, stimType) %>%
  summarise(mean_value <- mean(valueConfZ, na.rm = T) ) %>%
  data.frame()
...
dat <- droplevels(dat[dat$expPhase %in% c('acq','ext') & dat$bin1 == 3,])
...
# Define random effect structure, compare models with different random effects
# DECIDE ON REML
## Bayesian, with fixed effects
Sys.time()
b0 <- brm(eval(model_formula_1), data = dat, seed = seed_def)
b1 <- update(b0, . ~ . - (1 | idNum) + (1 + stimType | idNum), seed = seed_def, control = list(adapt_delta = .99))
b2 <- update(b0, . ~ . - (1 | idNum) + (1 + trial | idNum), seed = seed_def,control = list(adapt_delta = .99), iter = 8000)
b3 <- update(b0, . ~ . - (1 | idNum) + (1 + stimType + trial | idNum), seed = seed_def,control = list(adapt_delta = .99), iter = 8000)
...
# plot 1
# label contrasts
lab_names_pos <- c("acq_3,csp - acq_3,csm" = "CS+ vs CS- end acq",
                   "ext_3,csp - ext_3,csm" = "CS+ vs CS- end ext",
                   "acq_3,csp - ext_3,csp" = "CS+ acq vs ext",
                   "acq_3,csm - ext_3,csm" = "CS- acq vs ext")
...
# plot 2
# label contrasts
lab_names_pos <- c('acq_3,csp,AAAB,0 - acq_3,csm,AAAB,0' = 'CS+ vs CS- end acq cAAAB',
                   'ext_3,csp,AAAB,0 - ext_3,csm,AAAB,0' = 'CS+ vs CS- end ext cAAAB', # 'cs discrimination end extinction',
                   'acq_3,csp,AAAB,0 - ext_3,csp,AAAB,0' = 'CS+ acq vs ext cAAAB', # 'cs+ change acq to ext',
                   'acq_3,csm,AAAB,0 - ext_3,csm,AAAB,0' = 'CS- acq vs ext cAAAB',  # cs- change acq to ext
                   
                   # AABB, control group
                   'acq_3,csp,AABB,0 - acq_3,csm,AABB,0'= 'CS+ vs CS- end acq cAABB', # cs discrimination end acquisition
                   'ext_3,csp,AABB,0 - ext_3,csm,AABB,0'= 'CS+ vs CS- end ext cAABB', # cs discrimination end extinction
                   'acq_3,csp,AABB,0 - ext_3,csp,AABB,0'= 'CS+ acq vs ext cAABB', # cs+ change acq to ext
                   'acq_3,csm,AABB,0 - ext_3,csm,AABB,0' = 'CS- acq vs ext cAABB',  # cs- change acq to ext
                   
                   # AABA, control group
                   'acq_3,csp,AABA,0 - acq_3,csm,AABA,0'= 'CS+ vs CS- end acq cAABA', # cs discrimination end acquisition
                   'ext_3,csp,AABA,0 - ext_3,csm,AABA,0'= 'CS+ vs CS- end ext cAABA', # cs discrimination end extinction
                   'acq_3,csp,AABA,0 - ext_3,csp,AABA,0'= 'CS+ acq vs ext cAABA', # cs+ change acq to ext
                   'acq_3,csm,AABA,0 - ext_3,csm,AABA,0'= 'CS- acq vs ext cAABA',  # cs- change acq to ext
                   
                   # AAAB, reinstatement group
                   'acq_3,csp,AAAB,1 - acq_3,csm,AAAB,1' = 'CS+ vs CS- end acq rAAAB',
                   'ext_3,csp,AAAB,1 - ext_3,csm,AAAB,1' = 'CS+ vs CS- end ext rAAAB', # 'cs discrimination end extinction',
                   'acq_3,csp,AAAB,1 - ext_3,csp,AAAB,1' = 'CS+ acq vs ext rAAAB', # 'cs+ change acq to ext',
                   'acq_3,csm,AAAB,1 - ext_3,csm,AAAB,1' = 'CS- acq vs ext rAAAB',  # cs- change acq to ext
                   
                   # AABB, reinstatement group
                   'acq_3,csp,AABB,1 - acq_3,csm,AABB,1'= 'CS+ vs CS- end acq rAABB', # cs discrimination end acquisition
                   'ext_3,csp,AABB,1 - ext_3,csm,AABB,1'= 'CS+ vs CS- end ext rAABB', # cs discrimination end extinction
                   'acq_3,csp,AABB,1 - ext_3,csp,AABB,1'= 'CS+ acq vs ext rAABB', # cs+ change acq to ext
                   'acq_3,csm,AABB,1 - ext_3,csm,AABB,1' = 'CS- acq vs ext rAABB',  # cs- change acq to ext
                   
                   # AABA, reinstatement group
                   'acq_3,csp,AABA,1 - acq_3,csm,AABA,1'= 'CS+ vs CS- end acq rAABA', # cs discrimination end acquisition
                   'ext_3,csp,AABA,1 - ext_3,csm,AABA,1'= 'CS+ vs CS- end ext rAABA', # cs discrimination end extinction
                   'acq_3,csp,AABA,1 - ext_3,csp,AABA,1'= 'CS+ acq vs ext rAABA', # cs+ change acq to ext
                   'acq_3,csm,AABA,1 - ext_3,csm,AABA,1'= 'CS- acq vs ext rAABA')  # cs- change acq to ext
...
p_groups <- ggplot(dat_groups, aes(x = estimate, y = contrast)) + 
 ...
 # scale_alpha_discrete(range = c(.35, 1)) +
  ...
  #geom_point(data = dat_groups %>% filter(context == 'AAAB' & ri_group == 'reinstatement'), size = 3, color = 'white', aes(shape = type)) +
