...
source('../R_scripts/model_building/analysis/functions_to_load.R')
source('../R_scripts/model_building/analysis/contrasts_to_load.R')
...
# check GENERALIZED ri without ITIs
#dfps = droplevels(dfps[dfps$stimType %in% c('csp','csm'),])
...
drat <- drat %>% group_by(idNum) %>%
  mutate(valueConfZ = scale(valueConf)) %>%
  data.frame()
# select trials of interest
...
```{r echo=FALSE, warning = FALSE}
dr <- selDf(df_fps, cntxt = c('AAAB','AABB', 'AABA','AAAA'), numShocks = c(0,1))

# for CS types only
#dr <- droplevels(dr[dr$stimType %in% c('csp','csm'),])
...
Note that cases with missing data on at least one trial (n = `r length(id_w_miss)`) are completely removed from the analysis. 
This causes the pattern in the data to look different.

Testing interactions, type = 3
`r kable(out_anova, ditits = 3, format = 'markdown')` 
...
#Sys.time()
fitB0 <- brm(eval(model_formula_1), data = dataFrame, seed = seed_def)
#fitB5 <- update(fitB0, . ~ . - (1 | idNum) + (1 + stimType * trial | idNum))
#Sys.time()

fitB1 <- update(fitB0, . ~ . - (1 | idNum) + (1 + stimType | idNum), seed = seed_def)
fitB2 <- update(fitB0, . ~ . - (1 | idNum) + (1 + trial | idNum), seed = seed_def)
fitB3 <- update(fitB0, . ~ . - (1 | idNum) + (1 + stimType + trial | idNum), seed = seed_def)
fitB4 <- update(fitB0, . ~ . - (1 | idNum) + (1 + stimType * trial | idNum), seed = seed_def)
#Sys.time()
...
```{r echo=FALSE, warning = FALSE}
# estimates
tab_model(fitB0, fitB1, fitB2, fitB3, fitB4, fitB5)  
```
